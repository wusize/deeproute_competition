...
class DeeprouteDataset(Custom3DDataset):
    ...
    def get_ann_info(self, index):
        ...
        mode = self.split
        annos_path = self.data_root + mode +'/groundtruth/' + info + '.txt'
    ...
    def format_results(self,
        ...
        for i in range(len(results)):
            frame_result = {}
            object_list = []
            result_i = results[i] if 'pts_bbox' not in results[i] else results[i]['pts_bbox']
            for obj_num in range(len(result_i['scores_3d'])):
                obj={}
                obj["position"] = {}
                boxes_3d = result_i['boxes_3d'] \
                    if isinstance(result_i['boxes_3d'], torch.Tensor) else result_i['boxes_3d'].tensor
                obj["position"]["x"] = boxes_3d[obj_num, 0].item()
                obj["position"]["y"] = boxes_3d[obj_num, 1].item()
                obj["position"]["z"] = boxes_3d[obj_num, 2].item() + (0.5 * boxes_3d[obj_num, 5].item())
                obj["bounding_box"] = {}
                obj["bounding_box"]["width"] = boxes_3d[obj_num, 3].item()
                obj["bounding_box"]["length"] = boxes_3d[obj_num, 4].item()
                obj["bounding_box"]["height"] = boxes_3d[obj_num, 5].item()
                obj["score"] = result_i['scores_3d'][obj_num].item()
                obj["type"] = class_inverse_map[result_i['labels_3d'][obj_num].item()]
